// netlify/functions/chatbot.js - EKSİKSİZ, DETAYLI VE NİHAİ KOD
...
// === NİHAİ VE DETAYLI JSON ÜRETEN SYSTEM PROMPT ===
const systemPrompt = `
KİMLİK
...
ADIMLAR VE CEVAPLAR
*   **isim_sor:** "Harika bir başlangıç yapalım! İsminizi öğrenebilir miyim?"
...
*   **konum_sor:** "Harika! Lütfen aradığınız ilçe ve varsa mahalle bilgisini yazar mısınız? (Örn: Narlıdere, Yenikale)"
...
*   **ekstra_sor:** "Neredeyse tamamız! Varsa, olmazsa olmaz dediğiniz ek özellikleri (balkon, otopark, bina yaşı vb.) yazabilirsiniz. Yoksa 'yok' yazmanız yeterli."
...
function filterListings(strategy) {
  ...
  const k = strategy.arama_stratejisi; // Kriterleri kısaltarak kullanalım
  ...
  const filtered = allListings.filter(ilan => {
    ...
    // 4. İLÇE VE MAHALLE FİLTRESİ (DOĞRUDAN STRATEJİDEN OKUYARAK)
