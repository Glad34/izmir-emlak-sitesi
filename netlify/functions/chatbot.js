...
// === SİZİN GÖNDERDİĞİNİZ, BAŞARILI VE BASİTLEŞTİRİLMİŞ SYSTEM PROMPT ===
const systemPrompt = `
KİMLİK
...
"arama_stratejisi": {
    "isim": null, "amac": null, "konut_tipi": null, "konum": null, "butce": null,
    "oda_sayisi": null, "ek_notlar": null
}
}
`;

// === NİHAİ filterListings FONKSİYONU (BASİT STRATEJİ İÇİN) ===
function filterListings(strategy) {
  ...
  const filtered = allListings.filter(ilan => {
    ...
    // KONUM (İLÇE VE MAHALLE AYIRMA İŞİNİ KOD YAPIYOR)
    ...
    if (konumStr) {
        ...
        if (!ilce) ilce = konumStr; // Eğer ilçe bulamazsa, tamamını ilçe say
...
exports.handler = async function (event, context) {
    try {
        ...
        if (aiResponse.adim === 'onay_sonrasi') {
            ...
            const reportPrompt = `SİSTEM NOTU: Filtreleme yapıldı ve ${ilanSayisi} adet ilan bulundu. Şimdi GÖREV AKIŞI'ndaki 'onay_sonrasi' adımını bu bilgiye göre uygula.`;
